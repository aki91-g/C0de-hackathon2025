...
SQLAlchemyは、データベースにアクセス、操作できるようにするライブラリです。これにより、データベースのテーブルをpythonのclassとして扱えるようになります。<p>
...
動作確認はfastAPIのドキュメントで行います。<p>
今回のタスクではルーターの追加は行わないため、動作確認のために呼び出すエンドポイントは、external apis の GETメソッドのみです。<p>

動作確認で確認する事項は、大きく分けて3つあります。
1. エンドポイントが動いているか。
2. 外部APIからデータを取得できているか。
3. 意図した形式のレスポンスが帰ってきているか。

エンドポイントが動いていない場合は、 `app/services/external_api_service.py`を確認してください。（具体的にどのようなエラーが起こっているかを特定するために、事前にエラー処理を含めておくと、解消しやすくなります。）<p>
外部APIからデータを取得できていない場合は、バックエンドを立ち上げているターミナル(uv run python main.py を実行したターミナル)を開き、外部APIから返ってくるエラーコードに従って解消してください。<p>
意図した形式のレスポンスが帰ってきていない場合は、スキーマを確認してください。（スキーマに従わない場合はエラーとなるため、これは考えにくいと思われます）

エラーコードは、HTTP レスポンスステータスコードを参考にしてください。<p>
参照：https://developer.mozilla.org/ja/docs/Web/HTTP/Reference/Status
